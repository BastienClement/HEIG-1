...
\subsection{Structure de l'historique du dépôt}
...
Lorsque l'utilisateur enregistre une nouvelle version de son projet (appelée \textit{commit}), un \textit{snapshot} de l'état actuel des fichiers est enregistré dans l'historique du dépôt.
Par optimisation, lorsqu'un fichier n'a pas été modifié, le contenu du snapshot précédent est réutilisé tel quel. Les dernières version des fichier sur lesquels travail l'utilisateur (appelé le \textit{répertoire de travail}) sont à la fin de la chaine et formeront la prochaine révision du projet lors du commit.
...
\begin{enumerate}
	...
	l'adresse d'une information étant directement liée à son contenu, il est impossible qu'une corruption du dépôt passe inaperçue.
